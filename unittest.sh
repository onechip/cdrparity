...
if [ $(( `wc -c <test_01.tmp` % 2048 )) -ne 0 ]; then
    head -c $(( 2048 - (`wc -c <test_01.tmp` % 2048) )) /dev/zero >>test_01.tmp
fi

echo
cat test_00.tmp >test_02.tmp
echo cdrparity -s "$low_size"k -p test_02.tmp
./cdrparity -s "$low_size"k -p test_02.tmp
echo
...
echo cdrparity -s "$high_size"k -p test_04.tmp
./cdrparity -s "$high_size"k -p test_04.tmp
echo
